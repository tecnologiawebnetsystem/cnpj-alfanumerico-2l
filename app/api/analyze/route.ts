...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!body) {
      console.log("[v0] Step 2: ✗ Empty request body")
    ...
    console.log("[v0] Step 3: Getting current user from token...")
    const authHeader = request.headers.get("authorization")
    const user = await getUserFromToken(authHeader)

    if (!user) {
      console.log("[v0] Step 3: ✗ User not authenticated")
      return NextResponse.json({ error: "Não autenticado" }, { status: 401 })
    }

    console.log("[v0] Step 3: ✓ User authenticated:", user.email, "Role:", user.role)

    console.log("[v0] Step 4: Creating batch analysis...")
    ...
    try {
      ...
      if (rpcError) {
        console.log("[v0] Step 4: RPC failed, using direct insert...", rpcError.message)
        ...
        if (insertError) {
          console.log("[v0] Step 4: ✗ Failed to create batch", insertError)
          throw insertError
        }
      }

      console.log("[v0] Step 4: ✓ Batch created:", batchId)
    } catch (batchError: any) {
      console.log("[v0] Step 4: ✗ Batch creation failed", batchError)
      return NextResponse.json({ error: "Erro ao criar lote de análise", details: batchError.message }, { status: 500 })
    }

    console.log("[v0] Step 5: Fetching repositories from database...")
    ...
    if (reposError || !repos || repos.length === 0) {
      console.log("[v0] Step 5: ✗ No repositories found in database")
      ...
      try {
        ...
        if (syncError || !syncedRepos || syncedRepos.length === 0) {
          console.log("[v0] Step 5: ✗ Still no repositories after sync")
          return NextResponse.json({ error: "Não foi possível sincronizar repositórios" }, { status: 404 })
        }

        console.log("[v0] Step 5: ✓ Synced and loaded", syncedRepos.length, "repositories")
      } catch (syncErr: any) {
        console.log("[v0] Step 5: ✗ Sync failed:", syncErr.message)
      ...
      console.log("[v0] Step 5: ✓ Loaded", repos.length, "repositories")
    }

    console.log("[v0] Step 6: Loading search patterns...")
    ...
    if (!patterns.length) {
      try {
        ...
        if (dbPatterns && dbPatterns.length > 0) {
          patterns = dbPatterns
          console.log("[v0] Step 6: ✓ Loaded", patterns.length, "custom patterns")
        } else {
          patterns = [
          ...
          console.log("[v0] Step 6: ✓ Using default patterns")
        }
      } catch (patternError) {
        patterns = [
          { field_name: "CNPJ", pattern: "CNPJ", description: "CNPJ padrão" },
        ...
        console.log("[v0] Step 6: Using default patterns (error loading custom patterns)")
      }
    }

    console.log("[v0] Step 7: Initializing CNPJ detector...")
    const detector = new CNPJDetector(patterns.map((p) => p.field_name))
    console.log("[v0] Step 7: ✓ Detector initialized")

    console.log("[v0] Step 8: Starting repository analysis...")
    ...
    for (let i = 0; i < repos.length; i++) {
      const repo = repos[i]
      console.log(`[v0] Step 8.${i + 1}: Analyzing repository ${i + 1}/${repos.length}:`, repo.name)

      try {
        const analysisId = globalThis.crypto.randomUUID()

        console.log(`[v0] Step 8.${i + 1}.1: Creating analysis record...`)
        ...
        if (analysisError) {
          console.log(`[v0] Step 8.${i + 1}.1: ✗ Failed to create analysis`, analysisError)
          continue
        }

        console.log(`[v0] Step 8.${i + 1}.2: Fetching repository files...`)
        ...
        if (!azureToken) {
          console.log(`[v0] Step 8.${i + 1}.2: ✗ No Azure DevOps token`)
        ...
        if (!filesResponse.ok) {
          console.log(`[v0] Step 8.${i + 1}.2: ✗ Failed to fetch files`)
        ...
        console.log(`[v0] Step 8.${i + 1}.3: Analyzing ${files.length} files...`)
        ...
        for (const file of files) {
          try {
            ...
            if (results.length > 0) {
              console.log(`[v0] Found ${results.length} findings in ${file.path}`)
            ...
            console.log(`[v0] Error analyzing file ${file.path}:`, fileError)
          }
        }
        ...
        if (updateAnalysisError) {
          console.log(`[v0] Failed to update analysis status`, updateAnalysisError)
        }

        completedCount++
        const progress = Math.round((completedCount / repos.length) * 100)

        console.log(`[v0] Step 8.${i + 1}.4: Updating batch progress...`)
        try {
        ...
        console.log(`[v0] Step 8.${i + 1}: ✓ Repository ${i + 1}/${repos.length} completed (${progress}%)`)
      } catch (repoError: any) {
        console.log(`[v0] Step 8.${i + 1}: ✗ Error analyzing repository:`, repoError.message)
        continue
      }
    }

    console.log("[v0] Step 9: Finalizing batch analysis...")
    try {
    ...
    console.log("[v0] Step 9: ✓ Batch finalized")

    console.log("[v0] ========== ANALYZE API SUCCESS ==========")
    return NextResponse.json({
      success: true,
      batchId,
      stats: {

...
async function handleUpdate(request: NextRequest, params: Promise<{ id: string }>) {
  try {
    ...
    if (body.remaining_hours !== undefined) updateData.remaining_hours = body.remaining_hours
    ...
    if (body.status === "completed") {
      ...
      if (body.commit_hash) updateData.commit_hash = body.commit_hash
      if (body.pr_number) updateData.pr_number = body.pr_number

...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log("[v0] Querying tasks table...")
    ...
    if (includeRepo && tasks && tasks.length > 0) {
      ...
      console.log("[v0] Fetching repositories for", analysisIds.length, "analyses...")
    ...
    console.log(`[v0] Tasks enriched with user data and repositories`)
    console.log("[v0] === GET /api/client/tasks END ===")

    return NextResponse.json(tasksWithUsers)
  } catch (error) {
    console.error("[v0] CRITICAL Error fetching tasks:", error)
...
export async function POST(request: NextRequest) {
  try {
    console.log("[v0] === POST /api/client/tasks START ===")

    const supabase = createSupabaseServiceClient()
    const body = await request.json()

    console.log("[v0] Creating new task:", body.title)
    ...
    console.log("[v0] Task created successfully:", data.id)
    console.log("[v0] === POST /api/client/tasks END ===")

    return NextResponse.json(data)
  } catch (error) {
    console.error("[v0] CRITICAL Error creating task:", error)
...
export async function PUT(request: NextRequest) {
  try {
    console.log("[v0] === PUT /api/client/tasks START ===")
    const { searchParams } = new URL(request.url)
    ...
    console.log("[v0] Task updated successfully:", taskId)
    console.log("[v0] === PUT /api/client/tasks END ===")

    return NextResponse.json(data)
  } catch (error) {
    console.error("[v0] CRITICAL Error in update task API:", error)
    return NextResponse.json({ error: "Failed to update task" }, { status: 500 })
  }
}
